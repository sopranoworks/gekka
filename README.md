# gekka &nbsp;[![Version](https://img.shields.io/badge/version-1.0.0--rc5-blue)](https://github.com/sopranoworks/gekka)
...
## What's New in v1.0.0-rc5

This candidate is a **Persistence** pass: 4 commits since `v1.0.0-rc4`, no API breakage. It adds a new embedded persistence backend, fixes a real actor-messaging blocking defect, and hardens the (already-shipped) cross-process persistence forwarding with genuine two-OS-process proofs.

- **bbolt `DurableStateStore` backend — new** (`41e92b2`) — a new `persistence/bbolt` provider stores durable actor state in a single embedded [bbolt](https://github.com/etcd-io/bbolt) file, with no external database process — a good fit for lightweight, dependency-free deployments. It is registered as the `"bbolt"` plugin so it is HOCON-selectable exactly like the in-memory, SQL, and Redis backends; every `Upsert`/`Delete` commits and fsyncs its own transaction, so state survives a process restart. Proven by a genuine child-process restart-durability test (write, crash without a clean close, reopen the file, state + revision + concrete Go type all intact).
- **System-level `Send` no longer blocks on a full mailbox** (`166dcde`) — the non-networked `ActorSystem.Send`/`SendWithSender` path blocked indefinitely when the target mailbox was full, so one slow consumer could stall the caller forever. It now drops the message, publishes a `DeadLetter` (distinguishing `mailbox-full` from `mailbox-closed`), and returns an error — matching `Tell`'s drop semantics and the remote-ingress path. Regression tests included.
- **Cross-process journal + snapshot-store forwarding — proven across real OS processes** (`80ee8c4`, `336b4d2`) — the plugin-proxy off-mode forwarding that lets a `PersistentActor` use a journal / snapshot-store hosted on another node (over the same Artery transport as ordinary actor messaging) now has genuine two-OS-process proofs: a child process hosts the real store; the parent writes and replays (journal) or saves and loads (snapshot) through an off-mode proxy; and because neither side holds any local state, a correct round-trip is reachable only via real cross-process forwarding, never a false positive. The forwarding implementation itself predates this candidate.
- **Persistence backend dependency isolation — verified** — confirmed with real `go list -deps` that a consumer importing only `persistence/bbolt` pulls in no SQL driver, and one importing only `persistence/sql` pulls in no bbolt; the only shared persistence dependency is the standard library's `database/sql`.

The bbolt backend is the only new public surface in this candidate.
