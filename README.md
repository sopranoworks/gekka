...
- **Pekko Config Completeness — honest accounting** (Round-2 sessions 01–41 closed 2026-04-30, portable-pekko milestone sub-plans 1–8i closed 2026-05-05): the 2026-04-30 ✅-honesty pass surfaced 27 dishonest classifications (rows that parsed the HOCON value but only exposed it via an `Effective<X>()` getter that test code called); 16 sub-plan commits then wired each path onto a real production consumer. Sub-plan scope: typed-API misc (restart-stash-capacity, log-stashing, replicator-message-adapter unexpected-ask-timeout), sharded-daemon-process keep-alive + role, three discovery providers (`config`, `aggregate`, `pekko-dns`), typed receptionist (write-consistency / pruning / sharding), separate watch failure detector (`pekko.remote.watch-failure-detector.*`), reliable-delivery (chunking, flow-control window, only-flow-control, work-pulling buffer-size), external shard allocation client-timeout, multi-DC FD `expected-response-after`, sharding ddata replicator + coordinator-singleton overrides, sharding healthcheck `/health/ready` endpoint, Artery restart counters (inbound + outbound), idle/quarantine sweep schedulers, compression-table-manager production wiring, shutdown-flush + death-watch flush consumers. **Current state (post perfect-pekko Phase 10 closure, 2026-05-09): 302 ✅, 0 ⚠️, 0 ❌, 8 ☕ (JVM-only thread-pool / JMX / Scala-FQCN), 4 🚫 (JVM-only ssl.* + dispatch-FQCN swap shapes).** Phases 2–9 closed every ⚠️ row across the portable surface (receive buffer pools, sender-side system-message redelivery, multi-DC FD reachability margin, DistributedData per-CRDT serializer cache, sharding hill-climbing passivation optimizer, sharding lifecycle timers + ddata durable filter, sharding coordinator read/write-majority-plus, persistence-plugin-proxy cross-process mode). Phase 10 (2026-05-09) closes the final two ❌ rows (`pekko.loglevel`, `pekko.stdout-loglevel`) on the new slog-based logger foundation — see the next bullet. Phase 1 sub-commits 1.1–1.6 closed the entire mailbox subsystem: `UnboundedMailbox`, `BoundedMailbox`, `UnboundedControlAwareMailbox`, `BoundedControlAwareMailbox` factories ship in `actor/mailbox`; `Props.WithMailbox(name)` selects one; the `mailbox.RequiresControlAwareMessageQueueSemantics` marker triggers HOCON-driven binding with HARD-error mismatch validation; system-message priority at the actor cell is guaranteed via the two-channel dispatch from sub-commit 1.2. The Round-2 milestone closure remains accurate in its original taxonomy — 0 `🔒 HARDCODED`, 0 `❌ NO FEATURE` — but in the current 5-symbol scheme this is **not** "100% portable surface". See `docs/CONFIG_PATH.md` and `docs/LEFTWORKS.md §11`.
- **slog-based Logging Foundation — perfect-pekko Phase 10** (Project A sub-commits A1–A5.7 2026-05-07 → 2026-05-09; Project B closure 2026-05-09): the in-process logger is now permanently built on Go's stdlib `log/slog`. The new `logger/` package ships a composite handler with two filtered legs (`logger/handler.go`) — `mainLeg` gated by `MainLevelVar` (Pekko `loglevel` semantics) and `stdoutLeg` gated by `StdoutLevelVar` (Pekko `stdout-loglevel` semantics) — plus an `Install`/`Uninstall` lifecycle (`logger/install.go`) that atomically swaps the main leg without coordinating with concurrent emits, a Pekko-compatible level parser including the `OFF` sentinel `LevelOff = math.MaxInt32` (`logger/level.go`), and `Default()` / `WithSystem(name)` accessors. `Project A` migrated **every** in-process `log.Print*` and `fmt.Fprintf(os.Stderr, …)` callsite in production code to `logger.Default()` across cluster, internal/core, persistence, stream, actor, and management packages — the stdlib `log` import is fully eliminated from production code (only `examples/*/main.go` retain it). `Project B` (commit `14d1b30`) wires `cluster.go` `NewCluster` → `logger.Install` from `ClusterConfig.LogLevel` and `ClusterConfig.StdoutLogLevel` (parsed from `pekko.loglevel` / `pekko.stdout-loglevel`, with `akka.*` aliases and a `gekka.logging.level` fallback); empty values default to `"INFO"` / `"WARNING"` exclusively at the install call site (Pekko `reference.conf` defaults), so the parse site stays honest to "absent". `stdout-loglevel = "OFF"` maps to `LevelOff` and silences the stdout leg without disabling the main leg; runtime LevelVar changes are lock-free and observed on the next record. `level_binding_test.go` covers the HOCON-to-LevelVar binding end-to-end. Future external-log-server transport remains an additive enhancement that plugs into `logger.Install` `Options.Main` with no config-surface change. Closes the final two ❌ rows (125, 126) in `docs/CONFIG_PATH.md`; perfect-pekko-compat is one phase from full closure (Phase 11 — final honesty audit + Round-3 closure, docs only).
...
- **Integration-test runtime reliability** (2026-05-10) — four-part rework of how Go integration tests host the Scala/Akka fixtures, eliminating a long-standing 200 GB disk leak and three intermittent test failures. **(1) Fat-JAR launch path** (commits `e0746e7` … `c3b4634`): the `sbt-assembly 2.2.0` plugin is wired into `scala-server/`, producing two assembly JARs (`pekko-mains-assembly.jar` + `akka-mains-assembly.jar`); `test/jvmproc` gains `EnsureAssembly(t, p)` (sync.Once-memoized, mtime-fresh, `GEKKA_FORCE_ASSEMBLY=1` escape hatch) and `SpawnJava(t, ctx, jar, mainClass, mainArgs, opts)` so the four `sbt runMain` integration sites (`integration_pekko_test.go`, `integration_sbr_test.go`, `integration_test.go`, `integration_akka_test.go`) now launch `java -cp <jar> <Main>` directly. With sbt out of the runtime parent chain, jvmproc's SIGKILL no longer skips sbt's `BackgroundJobService` shutdown hook → no `bg-jobs/sbt_<hash>/` directory is created. Site #4 (`multi-jvm:test`) intentionally stays on sbt because the Akka MultiJvmPlugin is a test framework, not a `runMain` target. Verified: bg-jobs entry count is 297 → 297 across full integration runs (delta = 0). **(2) `assoc.remote` data race** (commit `4bcbacd`): the race detector reported `dispatch` reads racing with `handleHandshakeReq` writes on `GekkaAssociation.remote *UniqueAddress`. On multi-lane connections this caused corrupted protobuf field reads and downstream sharding-test flakiness. Converted the field to `atomic.Pointer[UniqueAddress]` — lock-free reads, race-safe writes — and updated every call site in `internal/core/association.go`, `internal/core/udp_artery_handler.go`, and the related test fixtures. `-race -count=10` on the previously-flaky `TestAdaptiveShardingRebalance` now emits zero `DATA RACE` warnings on this field. **(3) Adaptive-sharding rebalance determinism** (commit `13628dc`): `cluster.MetricsGossip` gains a `PublishNow()` method that synchronously samples the local collector and writes to the cluster-wide LWWMap, complementing the existing 5 s ticker. `TestAdaptiveShardingRebalance` calls `PublishNow()` immediately after `SetMockPressure` and then waits (via a polling helper) for both nodes' mock CPU values to replicate through DData (gossip-interval 200 ms) before entering the rebalance loop. Without this gate the loop frequently raced past its 30 s deadline against the 5 s metrics tick. Verified: 30/30 PASS without `-race`, 10/10 PASS under `-race`; per-iteration runtime fell from ~16 s to ~12 s because rebalance now fires on the first tick instead of the third. **(4) `TestCluster_GoDominantMixed` port selection** (commit `237d243`): the test's "find Scala port" loop iterated `cm.GetState().GetMembers()` filtering on port alone, but Go-3 (which had just `Leave()`d) lingered in the gossip member list as `Removed` long enough to be picked first; `DownMember` then no-op'd on Go-3 (already removed) and the real Scala node stayed `Up`, so the cluster sat at 4 Up forever and the 90 s convergence deadline expired. The lookup now filters on `MemberStatus_Up`, restoring the test's intent without weakening any assertion. Verified: 10/10 PASS in isolation (was 7/10).
